...
public unsafe struct msghdr
{
  ...
  public socklen_t msg_controllen
    {
    get => _msg_controllen.ToUInt32 ();
    set => _msg_controllen = (uint)value;
    }
  ...
  public int msg_iovlen
    {
    get => _msg_iovlen.ToInt32 ();
    set => _msg_iovlen = value;
    }
...
public struct cmsghdr
{
  ...
  public socklen_t cmsg_len
    {
    get => _cmsg_len.ToUInt32 ();
    set => _cmsg_len = (uint)value;
    }
...
public unsafe struct sockaddr_storage
{
  ...
  [field: FieldOffset (0)]
  public sa_family_t ss_family { get; set; }
}
...
public static unsafe partial class LibC
{
  ...
  public static int CMSG_ALIGN (int len) => (len + SizeOf.size_t - 1) & ~(SizeOf.size_t - 1);

  public static byte* CMSG_DATA (cmsghdr* cmsg) => (byte*)(cmsg + 1);

  public static cmsghdr* CMSG_FIRSTHDR (msghdr* mhdr)
  {
  ...
  public static int CMSG_LEN (int len) => CMSG_ALIGN (SizeOf.cmsghdr + len);

  public static cmsghdr* CMSG_NXTHDR (msghdr* mhdr, cmsghdr* cmsg)
  {
    if (cmsg->cmsg_len < SizeOf.cmsghdr || __CMSG_LEN (cmsg) + SizeOf.cmsghdr >= __MHDR_END (mhdr) - (byte*)cmsg)
    {
      return null;
    }
  ...
  public static int CMSG_SPACE (int len) => CMSG_ALIGN (len) + CMSG_ALIGN (SizeOf.cmsghdr);
  ...
  private static int __CMSG_LEN (cmsghdr* cmsg) => CMSG_ALIGN ((int)cmsg->cmsg_len);

  private static byte* __CMSG_NEXT (cmsghdr* cmsg) => (byte*)cmsg + __CMSG_LEN (cmsg);

  private static byte* __MHDR_END (msghdr* mhdr) => (byte*)mhdr->msg_control + mhdr->msg_controllen;
}
