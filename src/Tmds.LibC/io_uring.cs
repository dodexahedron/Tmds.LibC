...
public static unsafe partial class LibC
{
  public static int io_uring_enter (int fd, uint to_submit, uint min_complete, uint flags, sigset_t* sig) =>
    (int)syscall (__NR_io_uring_enter, fd, to_submit, min_complete, flags, sig, NSIG / 8);

  public static ushort IO_URING_OP_SUPPORTED => 1 << 0;

  public static int io_uring_register (int fd, uint opcode, void* arg, uint nr_args) => (int)syscall (__NR_io_uring_register, fd, opcode, arg, nr_args);

  public static int io_uring_setup (uint entries, io_uring_params* p) => (int)syscall (__NR_io_uring_setup, entries, p);
